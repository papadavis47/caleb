...
### Two API details NOT verified — confirm before trusting the plan's code

These two were written from reference docs, not run. If the compiler
disagrees, **the compiler is right**: keep the stated behavior and adjust
the call. Do not change the behavior to match a signature that compiles.

1. **`jiff::Timestamp::from_second(secs)`** in Task 3. Required behavior:
   UTC seconds → civil fields. If the constructor differs, find the jiff
   equivalent (check `~/.cargo/registry/src/*/jiff-0.2.35/src/`).
2. **The clap `version: ()` field** in Task 11. Required behavior: `-v` and
   `--version` print `caleb 0.1.0` and exit 0; `-V` is not required. If
   clap 4.6's derive rejects that field shape, implement the same behavior
   whatever way clap supports.

### Expected test counts are approximate

Each task states a running total ("Expected: 49 passed"). These are
estimates for orientation. A different total is not a failure — a failing
or erroring test is. Never edit a test to hit a stated number.
